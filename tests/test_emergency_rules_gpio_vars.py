...
def test_emergency_rule_validation_allows_string_in_active_alarms(tmp_path: Path) -> None:
    settings_dir = tmp_path / "settings"
    settings_dir.mkdir(parents=True, exist_ok=True)
    parser_path = settings_dir / "settings.json"
    parser_path.write_text(
        json.dumps(
            {
                "requests": [{"name": "hr", "fc": 3, "address": 0, "count": 1}],
                "fields": [{"name": "active_alarms", "type": "bitfield", "source": "hr", "address": 0, "bits": {}}],
            },
            ensure_ascii=False,
        )
        + "\n",
    ...
    ok, err = validate_emergency_rule_expression('"Ошибка чтения Modbus" in active_alarms', settings_path=parser_path)
    assert ok, err
