...
# Только создание файла .env в корне проекта (интерактивно или дефолты без TTY).
# Запуск приложения — отдельно: legacy/scripts/linux/run_blackbox.sh
...
if [ -f "$ENV_FILE" ]; then
  echo "Файл .env уже есть: $ENV_FILE"
  echo "Удалите его вручную, если нужно создать заново."
  exit 0
fi
