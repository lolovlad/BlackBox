...
# Служба запускается от root (см. legacy/deploy/systemd/blackbox.service).

PROJECT_ROOT="$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)"
SERVICE_SRC="$PROJECT_ROOT/legacy/deploy/systemd/blackbox.service"
SERVICE_DST="/etc/systemd/system/blackbox.service"
ENV_FILE="/etc/default/blackbox"

if [ "$(id -u)" -ne 0 ]; then
  echo "Run as root: sudo sh legacy/scripts/linux/install_systemd_service.sh"
  exit 1
fi
...
chmod +x "$PROJECT_ROOT/legacy/scripts/linux/create_env.sh" "$PROJECT_ROOT/legacy/scripts/linux/run_blackbox.sh"

mkdir -p /opt/blackbox
cp -a "$PROJECT_ROOT/." /opt/blackbox/
chown -R root:root /opt/blackbox
chmod +x /opt/blackbox/legacy/scripts/linux/create_env.sh /opt/blackbox/legacy/scripts/linux/run_blackbox.sh

if [ ! -f /opt/blackbox/.env ]; then
  echo "Creating default /opt/blackbox/.env (no TTY — defaults only)..."
  (cd /opt/blackbox && sh legacy/scripts/linux/create_env.sh)
fi
