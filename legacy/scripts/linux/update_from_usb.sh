...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "  sudo bash legacy/scripts/linux/update_from_usb.sh"
  exit 1
fi
...
chmod +x "$DST_DIR/legacy/scripts/linux/run_blackbox.sh" "$DST_DIR/legacy/scripts/linux/create_env.sh" || true
