...
Скрипт `legacy/scripts/linux/install_systemd_service.sh` (запускать от **root**):
...
```bash
chmod +x legacy/scripts/linux/install_systemd_service.sh legacy/scripts/linux/create_env.sh legacy/scripts/linux/run_blackbox.sh
sudo sh legacy/scripts/linux/install_systemd_service.sh
```
...
Отредактируйте **`legacy/deploy/systemd/blackbox.service`** (или `/etc/systemd/system/blackbox.service`):
...
```bash
sudo sh /opt/blackbox/legacy/scripts/linux/create_env.sh
```

(из каталога `/opt/blackbox`: `cd /opt/blackbox && sh legacy/scripts/linux/create_env.sh`)
