...
# Для развёртывания на устройстве см. legacy/DEPLOY_ON_DEVICE_RU.md и скрипты
# legacy/scripts/linux/create_env.sh + run_blackbox.sh.
#
...
if [ ! -d "legacy/migrations" ] || [ ! -f "legacy/migrations/env.py" ]; then
  uv run flask db --directory legacy/migrations init
fi

echo "[4/6] Applying database migrations..."
uv run flask db --directory legacy/migrations upgrade
...
then
  ...
  uv run flask db --directory legacy/migrations stamp base
  uv run flask db --directory legacy/migrations upgrade
fi
